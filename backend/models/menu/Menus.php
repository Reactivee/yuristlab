...
//            ],
    ...
    /**
     * Get Main Admin menu items
     * @return array
     */
    public static function getMainAdminMenu($key = 'main_admin_menu')
    {
        $data = [];
        if (($menu = self::findOne(['key' => $key, 'status' => self::STATUS_ACTIVE])) !== null) {
    ...
    public static function getMenu($key = 'admin_menu')
    {
        $data = [];
        if (($menu = self::findOne(['key' => $key, 'status' => self::STATUS_ACTIVE])) !== null) {
            ...
            if (!empty($models = MenuItems::getActiveMenuItemsByCondition(['menu_id' => $menu->id, 'parent_id' => NULL]))) {
                /** @var MenuItems $model */
                foreach ($models as $model) {
                    if (!empty($childs = self::getMenuItemChilds($model->id))) {
                        array_push($data, [
                            'title' => $model->label,
                            'icon' => $model->class,
    ...
    protected static function getMenuItemChilds($id)
    {
        $data = [];
