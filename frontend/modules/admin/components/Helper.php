...
class Helper
{
     ...
     * @param integer|User $user
     */
    public static function filter2($items, $user = null)
    {
        if ($user === null) {
            $user = Yii::$app->getUser();
        }
        return static::filterRecursive2($items, $user);
    }
     ...
     * @return array
     */
    protected static function filterRecursive2($items, $user)
    {
        $result = [];
        //d($items);
        foreach ($items as $i => $item) {
            ...
            $url = ArrayHelper::getValue($item, 'url', '#');
            d($url);
            $allow = is_array($url) ? static::checkRoute($url, [], $user) : true;
            d($item);
            d($allow);
            if (isset($item['childs']) && is_array($item['childs'])) {
                $subItems = self::filterRecursive2($item['childs'], $user);
                if (count($subItems)) {
                    $allow = true;
                }
                $item['childs'] = $subItems;
            }
            if ($allow && !($url == '' && empty($item['childs']))) {
                $result[$i] = $item;
            }
        }
        //d($result);
